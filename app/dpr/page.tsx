...
export default function DPRPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showToast({ title: 'DPR submitted', description: 'Your daily progress has been recorded.', variant: 'success' });
        ...
        <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-4">
            {dprs.slice(0, 10).map((dpr) => (
              <article key={dpr._id} className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-700">
