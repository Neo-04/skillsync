...
export default function AparPage() {
  ...
  const handleCreateApar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        alert('APAR created successfully!');
        setFormData({ year: new Date().getFullYear(), achievements: '', challenges: '', innovations: '' });
        setShowForm(false);
        fetchApars();
      } else {
        alert(`Failed to create APAR: ${data.error}`);
      }
    } catch (error) {
      console.error('Error creating APAR:', error);
      alert('Failed to create APAR');
    }
  };

  const handleSubmitApar = async (aparId: string) => {
    if (!confirm('Are you sure you want to submit this APAR for review?')) return;
    ...
    try {
      ...
      if (data.success) {
        alert('APAR submitted for review!');
        fetchApars();
      } else {
        alert(`Failed to submit APAR: ${data.error}`);
      }
    } catch (error) {
      console.error('Error submitting APAR:', error);
      alert('Failed to submit APAR');
    }
  };
  ...
  const handleReviewApar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        alert('APAR reviewed successfully!');
        setShowReviewModal(false);
        setSelectedApar(null);
        setReviewData({ reviewerComments: '', reviewerScore: 0 });
        fetchApars();
      } else {
        alert(`Failed to review APAR: ${data.error}`);
      }
    } catch (error) {
      console.error('Error reviewing APAR:', error);
      alert('Failed to review APAR');
    }
  };

  const handleFinalizeApar = async (aparId: string) => {
    if (!confirm('Are you sure you want to finalize this APAR? This action cannot be undone.')) return;
    ...
    try {
      ...
      if (data.success) {
        alert('APAR finalized!');
        fetchApars();
      } else {
        alert(`Failed to finalize APAR: ${data.error}`);
      }
    } catch (error) {
      console.error('Error finalizing APAR:', error);
      alert('Failed to finalize APAR');
    }
  };
...
const getStatusColor = (status: string) => {
  switch (status) {
    case 'finalized':
      return 'bg-black text-white';         // bright white
    case 'reviewed':
      return 'bg-black text-white';      // light gray
    case 'submitted':
      return 'bg-black text-white';      // slightly darker gray
    case 'draft':
      return 'bg-black text-white';      // darker gray
    default:
      return 'bg-black text-white';      // even darker gray
  }
};
