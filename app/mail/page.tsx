...
export default function MailPage() {
  ...
  const handleEnhanceMail = async () => {
    if (!formData.body && !formData.subject) return alert('Please write some content');
    ...
    try {
      ...
      alert(`Failed to enhance email: ${error.message}`);
    } finally {
      setEnhancing(false);
    }
  };
  ...
  const handleSendMail = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.to || !formData.subject || !formData.body) return alert('Fill all fields');
    if (!user) return;
